...
- Fable stays in the main session, where it plans, writes contracts, integrates shared files, and makes final calls. Sonnet is the floor for delegated work.
...
7. **Bound what comes back.** Give every agent a return budget and shape: at most 15 lines covering files changed, decisions made, anything that blocked it. Every line an agent returns lands in your context. Past about ten agents, run waves and carry a short written state between them.
8. **Verify the combined result yourself.** Edit the shared files, then lint/compile/test the whole. Agents verify their own piece, only you can verify the composition. Delegate the verification run (phpcs, PHPUnit, Playwright) to a `sonnet` agent that returns only failures, the main session decides the fixes. Browser verification follows the browser-check skill: the main session drives the Playwright MCP for the composition check, and each subagent runs its own headless script.
